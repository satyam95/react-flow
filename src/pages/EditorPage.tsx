...
const EditorPageContent = () => {
  ...
  const addNewNode = useCallback(
    (edgeId: string, nodeType: string) => {
      ...
      setEdges((eds) => {
        const updatedEdges = eds.filter((e) => e.id !== edgeId).concat(newEdges);
      ...
      >
        ...
        <Panel position="top-left">
          <div className="bg-white h-12 rounded-md shadow-blu py-3 px-6 flex justify-between gap-6 items-center">
            <Button variant="ghost" onClick={handleCancel} className="text-[#221F20] font-semibold text-base p-0 hover:bg-white cursor-pointer underline">
              {"<- Go Back"}
            </Button>
            <div className="text-[#221F20] font-semibold text-base">Untitled</div>
            <Button variant="ghost" size="icon" onClick={() => setSaveModal(!saveModal)}>
        ...
        <Panel position="bottom-left">
          <div className="bg-white h-10 border-2 border-[#E0E0E0] rounded-lg">
            <Button variant="ghost" size="icon" className="rounded-none w-10 h-full border-r-2 border-[#E0E0E0]">
        ...
        <Panel position="bottom-right">
          <div className="flex items-center bg-white h-10 border-2 border-[#E0E0E0] rounded-lg">
            ...
            <Button variant="ghost" size="icon" className="rounded-none w-10 h-full border-r-2 border-[#E0E0E0]" onClick={() => handleZoomChange([Math.max(0.1, zoom - 0.1)])}>
              <Minus size={20} className="text-black" />
            </Button>
            <div className="w-50 px-4">
              <Slider value={[zoom]} min={0.1} max={2} step={0.01} className="custom-slider" onValueChange={handleZoomChange} />
            </div>
            <Button variant="ghost" size="icon" className="rounded-none w-10 h-full border-l-2 border-[#E0E0E0]" onClick={() => handleZoomChange([Math.min(2, zoom + 0.1)])}>
            ...
            <div className="bg-white rounded-lg w-full max-w-[596px]">
              <div className="p-4">
                <div className="flex justify-between items-center py-2">
                  ...
                  <button className="text-black cursor-pointer" onClick={() => setSaveModal(!saveModal)}>
                    <X size={18} />
                  </button>
                </div>
                <div className="pt-6 pb-16">
                  <form className="space-y-6">
                    <div className="space-y-2">
                      <Label htmlFor="text" className="text-[#4F4F4F] font-normal text-xs">Name</Label>
                      <Input id="text" type="text" placeholder="Type here..." required className="h-[42px] border-[#E0E0E0] text-base placeholder:text-[#BDBDBD] bg-white placeholder:text-base shadow-none ring-0 focus-visible:shadow-none focus-visible:ring-0" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="description" className="text-[#4F4F4F] font-normal text-xs">Description</Label>
                      <Textarea id="description" placeholder="Type here..." required className="h-[42px] border-[#E0E0E0] text-base placeholder:text-[#BDBDBD] bg-white placeholder:text-base shadow-none ring-0 focus-visible:shadow-none focus-visible:ring-0" />
                    </div>
                  </form>
                </div>
              </div>
              <div className="p-4 flex justify-end items-center gap-4 dialog-shadow">
                <Button variant="outline" size="sm" className="text-white hover:text-white cursor-pointer font-medium text-sm py-1.5 px-3 rounded-sm bg-[#EE3425] hover:bg-[#EE3425]">
                  Save
                </Button>
              </div>
            </div>
